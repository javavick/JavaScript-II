...
runners.forEach((atr) => fullName.push(`${atr.first_name} ${atr.last_name}`));

console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map((atr) => atr.first_name.toUpperCase());

console.log(allCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter((atr) => atr.shirt_size === "L");
...
// Problem 1: It turns out that two out of the three representatives from Skinix donated an even numbered amount. This inconsistency is upsetting the event director a fair bit. The event director requests a lists of the three representatives so that they may present proof to Skinix of this incredibly dire situation.
let skinixReps = runners.filter((atr) => atr.company_name === "Skinix");

console.log(skinixReps);

// Problem 2: The event director needs the emails of every runner to add them to his super cool "Run Club Email Blast" mailing list.
let emailList = [];

runners.forEach((atr) => emailList.push(atr.email));

console.log(emailList);

// Problem 3: The event director would like a list of all runners' first name prefixed with the word "Li'l". They've been having a rough day planning this event and they think this list will help them cheer up a bit.
let lilReps = runners.map((atr) => `Li'l ${atr.first_name}`);
